...
func (uu *UserUpdate) Save(ctx context.Context) (int, error) {
	if v, ok := uu.mutation.Login(); ok {
		if err := user.LoginValidator(v); err != nil {
			return 0, &ValidationError{Name: "login", err: fmt.Errorf("ent: validator failed for field \"login\": %w", err)}
		}
	}
...
func (uuo *UserUpdateOne) Save(ctx context.Context) (*User, error) {
	if v, ok := uuo.mutation.Login(); ok {
		if err := user.LoginValidator(v); err != nil {
			return nil, &ValidationError{Name: "login", err: fmt.Errorf("ent: validator failed for field \"login\": %w", err)}
		}
	}
